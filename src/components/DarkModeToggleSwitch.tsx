      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className="flex gap-2 absolute left-0 right-0 z-50 mt-2 rounded-lg border border-primary-main bg-background shadow-md overflow-hidden p-2"
          >
            {themeOrder.map((item) => (
              <button
                key={item}
                ...
                className={`fcc gap-1 w-full px-3 h-10 text-sm transition-colors rounded-lg duration-300 
                  ${theme === item ? 'bg-primary-main text-background' : 'hover:bg-primary-main hover:text-background text-primary-main bg-primary-veryLight'}
                `}
